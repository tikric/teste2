...
object FilamentPriceService {
    ...
    // NOTE: SerpAPI key must be set via BuildConfig.SERP_API_KEY in build.gradle
    // or retrieved from the backend /api/quotations endpoint.
    ...
    suspend fun getCheapestFilaments(materialType: String): List<FilamentOffer> = withContext(Dispatchers.IO) {
        // Try SerpAPI if key is configured via BuildConfig
        val serpApiKey = try {
            val clazz = Class.forName("com.bambuzau3d.BuildConfig")
            val field = clazz.getField("SERP_API_KEY")
            (field.get(null) as? String)?.takeIf { it.isNotEmpty() && it != "MY_SERP_API_KEY" } ?: ""
        } catch (e: Exception) { "" }

        val requestUrls = if (serpApiKey.isNotEmpty()) listOf(
            "https://serpapi.com/search.json?engine=google_shopping&q=filamento+$materialType+1kg&google_domain=google.com.br&gl=br&hl=pt&tbs=p_ord:p&api_key=$serpApiKey"
        ) else emptyList()
