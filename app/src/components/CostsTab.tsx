...
export const CostsTab: React.FC<CostsTabProps> = ({
  ...
  const [expenseCategory, setExpenseCategory] = useState<'FILAMENTO' | 'EQUIPAMENTO' | 'ENERGIA' | 'EMBALAGEM' | 'FERRAMENTAS' | 'HARDWARE' | 'SERVICOS' | 'MARKETING' | 'IMPOSTOS' | 'OUTROS'>('EMBALAGEM');
  ...
  const getExpenseCategoryLabel = (category: string) => {
    switch (category) {
      ...
      case 'FILAMENTO': return '🧵 Filamento Extra';
          ...
          {showAddProductManualForm && (
            <form onSubmit={handleSaveManualProduct} className="p-5 bg-[#151917] border border-[#232B27] rounded-2xl space-y-4 animate-fade-in" id="manual_catalog_product_form">
              <div className="flex items-center justify-between border-b border-[#232B27]/40 pb-2">
                <h4 className="text-sm font-bold text-[#95BBA2] uppercase flex items-center gap-1.5">
                  <Plus className="h-4 w-4" />
                  Cadastrar Novo Produto Manualmente
                </h4>
                <button
                  type="button"
                  onClick={() => setShowAddProductManualForm(false)}
                          ...
                          <div className="space-y-1">
                            ...
                            <div className="flex flex-wrap gap-1 text-[9px]">
                              {item.filamentsUsed.map((f, idx) => {
                                ...
                                return (
                                  <span key={idx} className="bg-teal-500/10 text-teal-300 px-1.5 py-0.5 rounded border border-teal-500/20 font-mono">
              ...
              {[...filamentStocks].sort((a, b) => {
                ...
                return (
                  <div key={fil.id} className={`p-3 bg-[#0C0E0D] border rounded-xl flex items-center justify-between gap-4 transition-all duration-300 ${
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-1.5">
                        <span className="w-2.5 h-2.5 rounded-full inline-block" style={{ backgroundColor: fil.type === 'PLA' ? 'var(--brand-primary)' : fil.type === 'PETG' ? 'var(--brand-accent)' : '#D35C5C' }} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-[#151917] border border-[#232B27] p-5 rounded-2xl space-y-4">
              ...
              <form onSubmit={handleAddExpenseLocal} className="space-y-3 bg-black/30 p-3.5 border border-[#232B27]/80 rounded-xl">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="space-y-0.5 col-span-2">
                    <label className="text-[9px] text-[#8BA58D] block">Valor R$</label>
                    <input
                      type="number"
                      step="0.01"
                  ...
                  <div className="space-y-0.5">
                    <label className="text-[9px] text-[#8BA58D] block">Qtd</label>
                    <input
                      type="number"
                      min="1"
                      value={expenseQty}
                      onChange={(e) => setExpenseQty(parseInt(e.target.value) || 1)}
                      className="w-full bg-[#0C0E0D] border border-[#232B27] rounded-xl px-3 py-1.5 text-xs text-white font-mono outline-none"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="space-y-0.5 col-span-2">
                    <label className="text-[9px] text-[#8BA58D] block">Categoria do Gasto (Sem ser estoque regular)</label>
                    <select
                      value={expenseCategory}
                      onChange={(e: any) => setExpenseCategory(e.target.value)}
                      className="w-full bg-[#0C0E0D] border border-[#232B27] rounded-xl px-2.5 py-1.5 text-xs text-white outline-none font-mono"
                    >
                      <option value="EMBALAGEM">📦 Embalagens & Caixas</option>
                      <option value="EQUIPAMENTO">⚙️ Impressoras, Peças & Upgrades</option>
                      <option value="ENERGIA">⚡ Infraestrutura & Energia Elétrica</option>
                      <option value="FERRAMENTAS">🛠️ Ferramentas & Acessórios</option>
                      <option value="HARDWARE">🔩 Ferragens & Fixadores (M3, Ímãs...)</option>
                      <option value="SERVICOS">💻 Serviços & Softwares (Fatiadores, Cloud)</option>
                      <option value="MARKETING">📣 Anúncios, Marketing & Tráfego</option>
                      <option value="IMPOSTOS">💸 Impostos & Taxas de Venda</option>
                      <option value="FILAMENTO">🧵 Filamento Extra / Avulso</option>
                      <option value="OUTROS">🏷️ Outros Custos Gerais</option>
                    </select>
                    <p className="text-[8px] text-[#8BA58D]/70 font-mono leading-normal mt-1">
                      💡 Filamentos e Insumos diretos das peças devem ser controlados no **Estoque** para calcular perdas e peso real.
                    </p>
                  </div>
                </div>
              ...
              <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
                {expenses.length === 0 ? (
                  <div className="p-8 text-center text-[#8BA58D] border border-dashed border-[#232B27]/40 rounded-xl">
                    <p className="text-xs italic font-mono">Nenhum gasto lançado este mês.</p>
                  </div>
                ) : (
                  [...expenses].reverse().map(item => (
                    <div key={item.id} className="p-3 bg-[#0C0E0D] border border-[#232B27]/70 rounded-xl flex items-center justify-between gap-3">
                      <div className="space-y-1">
                        ...
                        <div className="text-[9px] text-[#8BA58D] font-mono">
                          {item.qty} x R$ {item.amount.toFixed(2)} — {new Date(item.date).toLocaleDateString('pt-BR')}
                        </div>
                      </div>

                      <div className="flex items-center gap-3">
                        <span className="text-sm font-black font-mono text-red-400">
                          R$ {(item.amount * item.qty).toFixed(2)}
                        </span>
                        <button
                          onClick={() => onDeleteExpense(item.id)}
                          className="text-red-400 hover:text-red-500 p-1 rounded hover:bg-red-500/10 transition cursor-pointer"
                        >
              ...
              <div className="p-4 bg-black/40 border border-emerald-500/20 rounded-xl text-center">
                <span className="text-[8.5px] uppercase tracking-wider text-emerald-400 font-bold block">Total Efetivado em Materiais</span>
                <p className="text-xl font-bold font-mono text-white">
                  R$ {expenses.reduce((sum, e) => sum + (e.amount * e.qty), 0).toFixed(2)}
                </p>
              </div>
            </div>
