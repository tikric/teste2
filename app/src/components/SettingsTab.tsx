...
export const SettingsTab: React.FC<SettingsTabProps> = ({
  ...
  const [updateVersion, setUpdateVersion] = useState(() => {
    return localStorage.getItem('bambuzau_update_version') || '3.2.3.5';
  });
  ...
  const [localAnthropicKey, setLocalAnthropicKey] = useState(() => {
    return safeStorage.getItem('bambuzau_custom_anthropic_key', '');
  });
  ...
  React.useEffect(() => {
    const handleKeysUpdated = () => {
      ...
      setLocalAnthropicKey(safeStorage.getItem('bambuzau_custom_anthropic_key', ''));
  ...
  return (
    <div className="space-y-6" id="settings_tab_container">
      ...
      <div className="p-6 bg-[#151917] border border-[#232B27] rounded-3xl relative overflow-hidden space-y-5 my-6 animate-fade-in" id="card-unified-api-keys">
        ...
        {/* 3. ANTHROPIC (CLAUDE) API KEY - web search grounding for price quotes */}
        <div className="p-4 bg-[#0A0D0B] border border-[#232B27] rounded-2xl space-y-3">
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              <label className="text-xs font-bold text-gray-200 flex items-center gap-1 font-sans">
                <Search className="h-3.5 w-3.5 text-orange-400" />
                Chave de API da Anthropic / Claude (Super Pesquisa de Preços)
              </label>
              <p className="text-[10px] text-[#8BA58D]">Habilita busca web em tempo real via Claude para encontrar os menores preços de filamentos. Recomendado!</p>
            </div>
            <a href="https://console.anthropic.com/settings/keys" target="_blank" rel="noreferrer" className="text-[10px] text-orange-400 hover:underline font-semibold font-sans flex items-center gap-0.5">
              Criar Chave ↗
            </a>
          </div>
          <div className="flex flex-col sm:flex-row gap-2.5">
            <div className="relative flex-grow flex">
              <input
                ...
                placeholder="Cole sua chave Anthropic aqui (sk-ant-...)"
                value={localAnthropicKey}
                onChange={(e) => setLocalAnthropicKey(e.target.value)}
                className="flex-grow bg-[#151917] border border-[#232B27] pl-3.5 pr-10 py-2.5 rounded-xl text-xs text-white placeholder-zinc-800 hover:border-[#38463F] focus:border-[#f97316] outline-none font-mono"
            ...
            <button
              ...
              onClick={() => {
                try {
                  const trimmedKey = (localAnthropicKey || '').trim();
                  setLocalAnthropicKey(trimmedKey);
                  const check = validateApiKeyFormat(trimmedKey);
                  if (!check.isValid) {
                    showError(check.reason || 'Chave Anthropic inválida!');
                    return;
                  }
                  safeStorage.setItem('bambuzau_custom_anthropic_key', trimmedKey);
                  window.dispatchEvent(new Event('bambuzau_keys_updated'));
                  showSuccess('Chave da Anthropic salva! Super Pesquisa de Preços com Claude ativada 🚀');
                } catch (e: any) {
                  showError('Erro ao salvar chave Anthropic: ' + e.message);
                }
              }}
              className="px-5 py-2.5 bg-orange-600 hover:bg-orange-500 text-white font-black text-xs rounded-xl transition cursor-pointer select-none shrink-0"
              id="btn-save-anthropic"
            >
              Salvar Claude
            </button>
          </div>
          ...
          <div className="pt-2 border-t border-[#232B27]/40">
            ...
            {showClipboardBackup && (
              <div className="mt-3 bg-[#0C0E0D] border border-purple-500/10 p-3.5 rounded-xl space-y-3 animate-fade-in">
                <p className="text-[10px] text-[#8BA58D] leading-relaxed">
                  Bypass de arquivos para Celulares e WebViews! Trata as informações do Ateliê como um bloco de texto que você copia e cola à vontade. Versão 3.2.3.5.
                </p>
          ...
          <div className="space-y-3 max-h-[240px] overflow-y-auto pr-1">
            <div className="p-2.5 bg-[#0C0E0D] border-l-2 border-emerald-500 rounded-r-xl space-y-1 font-sans">
              <div className="flex justify-between items-center text-[10px]">
                <strong className="text-white text-xs font-black">Versão 3.2.3.5 (Atual)</strong>
          ...
          {(() => {
            const getLocalAppVer = () => {
              if (typeof window !== 'undefined') {
                ...
                const nv = params.get('nativeVersion');
                if (nv) return nv;
              }
              return '3.2.3.5';
            };
            ...
            return (
              <div className="p-4 bg-[#0A0D0B] border border-[#232B27] rounded-xl space-y-3 text-xs">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 font-mono text-[11px]">
                  <div>
                    <span className="text-[#8BA58D] text-[10px]">Sua Versão (Deste Dispositivo):</span>
