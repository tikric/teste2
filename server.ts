...
function isGeminiRateLimited(customKey?: string): boolean {
  // Se o usuário forneceu uma chave própria válida (que é diferente da do servidor que possa ter sido bloqueada/vazada),
  // ele não deve ser afetado pelo circuito limitador global.
  if (isValidApiKey(customKey)) {
    const serverKey = process.env.GEMINI_API_KEY ? process.env.GEMINI_API_KEY.trim() : "";
    if (customKey!.trim() !== serverKey) {
      return false; // Permite prosseguir imediatamente se for chave customizada própria
    }
  }
...
const quotationsCache = new Map<string, { timestamp: number; data: any }>();
const CACHE_TTL_MS = 20 * 60 * 1000; // 20 minutes server-side cache
...
async function startServer() {
  ...
  app.get("/api/quotations", async (req, res) => {
    try {
      ...
      let cacheKey = "all_quotations";
      if (customQ && customQ.trim()) {
        cacheKey = `q_${customQ.trim().toLowerCase()}`;
      } else if (type) {
        cacheKey = `type_${type.toLowerCase()}`;
      }

      const cached = quotationsCache.get(cacheKey);
      if (cached && (Date.now() - cached.timestamp) < CACHE_TTL_MS) {
        console.log(`[Cache Hit] Returning cached result for cotações key: ${cacheKey}`);
      ...
      const fetchQuoteMultiEngine = async (materialType: string, queryStr: string) => {
        ...
        const backfillToMinimum = (offers: any[]) => {
          ...
          if (result.length < 5) {
            for (const fallbackOffer of fb) {
              ...
              // Precise verification of real duplicates: check same store AND identical starting product name
        ...
        // If no internet resources are available from other search feeds, try AI Google Search Grounding dynamically
        if (serpResults.length === 0 && tavilyResults.length === 0 && jinaResults.length === 0) {
          if (isValidApiKey(geminiApiKey) && !isGeminiRateLimited(geminiApiKey)) {
            try {
              console.log(`[AI Search Grounding] Sourcing real-time Google Search grounding with Gemini for: ${materialType}`);
...
Retorne APENAS um array JSON limpo ordenado estritamente por preço menor para maior no formato:
[
  {
    "storeName": "Nome simplificado da loja",
    "productName": "Título amigável completo do produto real carretel 1kg",
    "price": 89.90, // valor numérico real/realista
    "rating": 4.8,
    "buyUrl": "url real da oferta ou da busca de mercado"
  }
]
Por favor, responda APENAS o JSON. Sem notas explicativas, sem markdown codeblocks fora do JSON.`,
                config: {
                  tools: [{ googleSearch: {} }]
                }
              });
              ...
              if (textResult.trim()) {
                ...
                if (Array.isArray(resArray) && resArray.length > 0) {
                  ...
                  console.log(`[AI Search Grounding] Success! Found ${finalOfs.length} offers for ${materialType}`);
                  return {
                    type: materialType,
              ...
              console.log(`[AI Search Grounding Alert] Sourcing offline reference list for ${materialType}.`);
              handleGeminiError(groundingErr);
            }
          } else if (isGeminiRateLimited(geminiApiKey)) {
            console.log(`[AI Search Grounding Skipped] Gemini is currently in cooldown/rate-limited for ${materialType}. Using static fallback.`);
          }
...
4. Se o resultado bruto faltar fontes confiáveis para fechar 5 itens, use fontes confiáveis tradicionais de filamento com preços de mercado realistas para completar as 5 opções com maestria.`;
        ...
        if (!filterSuccess && isValidApiKey(geminiApiKey) && !isGeminiRateLimited(geminiApiKey)) {
          try {
      ...
      if (customQ && customQ.trim()) {
        ...
        const responseData = [singleResult];
        quotationsCache.set(cacheKey, { timestamp: Date.now(), data: responseData });
      ...
      if (type && qMap[type]) {
        ...
        // Sequentially query to prevent simultaneous API fires and rate limits (429 RESOURCE_EXHAUSTED)
        const results = [];
        for (const mat of ["PLA", "PETG", "TPU"]) {
          const resValue = await fetchQuoteMultiEngine(mat, qMap[mat]);
          results.push(resValue);
          // Wait 500ms between each material search to maintain high API reliability
          await new Promise(resolve => setTimeout(resolve, 500));
        }
        quotationsCache.set(cacheKey, { timestamp: Date.now(), data: results });
...
2. Seja profissional e persuasivo nos pitches comerciais.`;
      ...
      // Try Gemini fallback
      if (!leadSuccess && isValidApiKey(geminiApiKey) && !isGeminiRateLimited(geminiApiKey)) {
        try {
  ...
  app.post("/api/ok-loja", async (req, res) => {
    try {
      ...
      if (!isValidApiKey(groqApiKey) && !isValidApiKey(geminiApiKey)) {
        return res.status(400).json({ 
          error: "Nenhuma chave de API configurada. Cadastre sua GEMINI_API_KEY ou sua GROQ_API_KEY na aba de Configurações!" 
        });
      }
      ...
      async function tryGemini(): Promise<boolean> {
        ...
        if (isGeminiRateLimited(geminiApiKey)) {
      ...
      if (provider === "groq") {
        if (!isValidApiKey(groqApiKey)) {
          // If Groq is missing but Gemini is configured, auto-failover directly!
          if (isValidApiKey(geminiApiKey)) {
            console.warn("[Ok Loja Fallover] Groq key missing/invalid, falling back immediately to Gemini.");
            await tryGemini();
          } else {
            return res.status(400).json({ 
              error: "A chave API da Groq (GROQ_API_KEY) não está configurada nos Secrets ou no Ateliê local. Insira sua chave Groq nas Configurações!" 
            });
          }
        } else {
          // Try Groq first, then failover to Gemini
          const groqOk = await tryGroq();
          if (!groqOk && isValidApiKey(geminiApiKey)) {
            console.warn("[Ok Loja Failover] Groq falhou ao responder. Iniciando recuperação com Gemini...");
            await tryGemini();
          }
        }
      } else {
        // Default: Gemini first, then Groq (failover)
        if (!isValidApiKey(geminiApiKey)) {
          // If Gemini key is missing/invalid, check if we have Groq configured as fallback directly!
          if (isValidApiKey(groqApiKey)) {
            console.warn("[Ok Loja Fallover] Gemini key missing/invalid, falling back immediately to Groq.");
            await tryGroq();
          } else {
            return res.status(400).json({
              error: "A chave API do Gemini (GEMINI_API_KEY) não está configurada nos Secrets do Projeto ou nas Configurações locais. Cole sua chave na aba Configurações!"
            });
          }
        } else {
          const geminiOk = await tryGemini();
          if (!geminiOk && isValidApiKey(groqApiKey)) {
            console.warn("[Ok Loja Failover] Gemini falhou com 503 ou limites. Iniciando recuperação com Groq...");
            await tryGroq();
          }
        }
      }
